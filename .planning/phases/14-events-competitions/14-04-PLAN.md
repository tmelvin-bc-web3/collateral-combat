...
class TournamentManager {
  ...
  private async checkReadyMatches(tournamentId: string): Promise<void> {
    ...
    for (const match of readyMatches) {
      ...
      try {
        // Create tournament battle via battleManager
        const battle = await battleManager.createBattle({
          player1Wallet: match.player1Wallet,
          player2Wallet: match.player2Wallet,
          entryFeeLamports: 0, // Entry fees already collected at registration
          duration: 300000, // 5 minute battles
          mode: 'tournament',
          tournamentMatchId: match.id,
        });

        // Link the battle to this match
        db.linkBattleToMatch(match.id, battle.id);

        logger.info('Tournament battle created', {
          matchId: match.id,
          battleId: battle.id,
        ...
        this.emit({ type: 'match_started', tournamentId, data: { matchId: match.id, battleId: battle.id } });
      } catch (err) {
        logger.error('Failed to create tournament battle', {
          matchId: match.id,
          error: String(err)
        });
      }
...
5. Connect battle completion to tournament advancement by subscribing tournamentManager to battleManager events:
```typescript
// After tournamentManager.initialize()
battleManager.onBattleComplete((battle) => {
  if (battle.tournamentMatchId) {
    const winnerWallet = battle.winnerId; // battleManager sets winnerId on completion
    if (winnerWallet) {
...
Note: The battleManager.onBattleComplete callback pattern is used for any post-battle processing.
If battleManager doesn't have this hook yet, add it as part of Task 3:
```typescript
// In battleManager.ts
private battleCompleteListeners: ((battle: Battle) => void)[] = [];

public onBattleComplete(listener: (battle: Battle) => void): void {
  this.battleCompleteListeners.push(listener);
}

// Call in settleBattle() after determining winner:
for (const listener of this.battleCompleteListeners) {
  try { listener(battle); } catch (err) { logger.error('Battle complete listener error', { error: String(err) }); }
}
```
  </action>
