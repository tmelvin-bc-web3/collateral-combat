...
<task type="auto">
  <name>Task 1: Install Satori and Sharp dependencies with Inter fonts</name>
  <files>backend/package.json, backend/assets/fonts/</files>
  <action>
...
3. Create fonts directory and download Inter fonts:
   ```bash
   mkdir -p backend/assets/fonts
   ```

4. Download Inter fonts from GitHub releases (these are the official variable font builds):
   ```bash
   # Download Inter Regular (weight 400)
   curl -L -o backend/assets/fonts/Inter-Regular.ttf \
     "https://github.com/rsms/inter/raw/v4.0/extras/ttf/Inter-Regular.ttf"

   # Download Inter Bold (weight 700)
   curl -L -o backend/assets/fonts/Inter-Bold.ttf \
     "https://github.com/rsms/inter/raw/v4.0/extras/ttf/Inter-Bold.ttf"
   ```

   Alternative if above URLs change:
   ```bash
   # From Google Fonts API (requires extraction from zip):
   curl -L -o /tmp/inter.zip "https://fonts.google.com/download?family=Inter"
   unzip -j /tmp/inter.zip "static/Inter-Regular.ttf" "static/Inter-Bold.ttf" -d backend/assets/fonts/
   ```

5. Verify fonts downloaded:
   ```bash
   ls -la backend/assets/fonts/
   # Should show Inter-Regular.ttf and Inter-Bold.ttf
   ```
  </action>
  <verify>
    - `grep "satori" backend/package.json` shows satori installed
    - `grep "sharp" backend/package.json` shows sharp installed
    - `ls backend/assets/fonts/Inter-Regular.ttf` exists
    - `ls backend/assets/fonts/Inter-Bold.ttf` exists
  </verify>
  <done>Satori and Sharp installed, Inter font files downloaded to backend/assets/fonts/</done>
</task>
...
4. Font files exist in backend/assets/fonts/ (Inter-Regular.ttf, Inter-Bold.ttf)
...
- Inter fonts downloaded via curl to backend/assets/fonts/
