...
must_haves:
  ...
  key_links:
    ...
    - from: "spectator_battle_update WebSocket"
      to: "QuickBetStrip odds prop"
      via: "parent state update triggers re-render"
      pattern: "setSelectedBattle.*spectator_battle_update"
---
...
**Important - Odds update flow:**
QuickBetStrip receives `odds` as a prop from the parent spectate page. The parent subscribes to
`spectator_battle_update` WebSocket events which update `selectedBattle` state. When selectedBattle
changes, QuickBetStrip re-renders with fresh odds via props. This is sufficient for live updates -
no direct WebSocket subscription needed in this component.
...
- Receives live odds via props from parent (spectator_battle_update → setSelectedBattle → re-render)
  </action>
  <verify>
```bash
...
<task type="auto">
  ...
  <action>
Add QuickBetStrip to the spectate page. The integration point is lines 321-328 where SpectatorView
is rendered when selectedBattle is set.
...
2. Find the selectedBattle return block (lines 321-328):
```tsx
if (selectedBattle) {
  return (
    <SpectatorView
...
3. Wrap in a fragment and add QuickBetStrip as sibling:
```tsx
if (selectedBattle) {
  return (
    <>
...
**Odds update flow:**
- The spectate page already subscribes to `spectator_battle_update` WebSocket events (line 206-213)
- These events update `selectedBattle` via `setSelectedBattle`
- When selectedBattle updates, both SpectatorView and QuickBetStrip re-render with fresh data
- selectedBattle has type `LiveBattle` which includes `.odds` property of type `BattleOdds`
- walletAddress is available at component level (line 164)

This means QuickBetStrip receives live odds automatically via React's re-render cycle -
no additional WebSocket subscription needed in the component itself.
  </action>
  <verify>
```bash
cd /Users/taylermelvin/Desktop/sol-battles/web && pnpm build 2>&1 | head -50
