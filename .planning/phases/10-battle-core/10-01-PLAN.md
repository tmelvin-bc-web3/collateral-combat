...
<task type="auto">
  <name>Task 2: Integrate ELO into matchmaking</name>
  <files>
    backend/src/services/battleManager.ts
  </files>
  <action>
...
Manually test: Start backend, check that matchmaking logs include tier in queue key
  ...
  <done>
Matchmaking uses ELO tiers, new players with <10 battles get 'protected' tier, ELO updates after battle end
  </done>
</task>
...
4. Matchmaking key includes ELO tier
5. Battle end updates both players' ELO ratings
6. API endpoint returns ELO data for any wallet
</success_criteria>
