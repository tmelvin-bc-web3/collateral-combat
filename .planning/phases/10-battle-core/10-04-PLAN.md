...
4. Modify updateAccountValue() to check for instant loss after liquidations:
```typescript
...
5. Modify updateAllAccounts() to check for instant loss:
```typescript
...
TypeScript compiles: `cd backend && npm run typecheck`
  ...
  <done>
Zero capital triggers instant battle loss, winner determined automatically, battle ends early with liquidation reason
  </done>
</task>
