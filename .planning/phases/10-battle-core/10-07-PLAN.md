...
<task type="auto">
  <name>Task 5: Implement backend accept_challenge socket handler</name>
  <files>
    backend/src/index.ts
  </files>
  <action>
Add the backend WebSocket handler for 'accept_challenge' event. This completes the frontend-backend wiring.

Find the socket event handlers section in backend/src/index.ts (around line 2395 where challenge notification handlers are) and add:
...
socket.on('accept_challenge', async (data: { code: string; walletAddress: string }) => {
  ...
  try {
    ...
    // Create the battle via battleManager
    const battleConfig = {
    ...
    // Start the battle with both players
...
Note: If battleManager doesn't have a `createBattleFromChallenge` method, you may need to use existing methods or add one. The pattern is:
1. Create battle with config from challenge
2. Add both challenger and accepter as players
3. Transition to 'waiting' or 'active' status depending on ready check requirements

Check battleManager.ts for existing methods like `createBattle` or `addPlayerToBattle` and compose appropriately.
  </action>
  <verify>
TypeScript compiles: `cd backend && npm run typecheck`
Backend starts without errors: `cd backend && npm run dev`
Socket handler exists: grep for "socket.on('accept_challenge'" in backend/src/index.ts
  </verify>
  <done>
Backend handles 'accept_challenge' socket event, validates challenge, creates battle from challenge parameters, notifies both players via WebSocket
  </done>
</task>

<task type="checkpoint:human-verify" gate="blocking">
  <what-built>Challenge board page with listing, filtering, creation, direct challenges, full matchmaking/challenge acceptance wiring, and backend socket handler</what-built>
  <how-to-verify>
...
10. Backend socket.on('accept_challenge') handler creates battle from challenge
</success_criteria>
