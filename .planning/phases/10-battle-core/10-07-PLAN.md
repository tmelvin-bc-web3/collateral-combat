...
must_haves:
  truths:
    ...
    - "Backend handles accept_challenge socket event and creates battle"
  artifacts:
  ...
  key_links:
    ...
    - from: "backend/src/index.ts"
      to: "socket.on('accept_challenge')"
      via: "WebSocket handler for challenge acceptance"
      pattern: "socket.on.*accept_challenge"
    - from: "backend/src/index.ts queue_matchmaking handler"
      to: "battleManager.queueForMatchmaking"
      via: "Handler calls queueForMatchmaking which uses async getMatchmakingKey with ELO tier"
      pattern: "battleManager\\.queueForMatchmaking"
---
...
Output: Challenge board page, challenge card components, create challenge modal, battle page challenge handling, matchmaking wiring, backend socket handler
</objective>
...
@.planning/phases/10-battle-core/10-01-SUMMARY.md
...
<task type="auto">
  <name>Task 4: Wire matchmaking queue from BattleLobby UI with ELO tier verification</name>
  <files>
    web/src/components/BattleLobby.tsx
  </files>
  <action>
...
const handleFindMatch = () => {
  ...
  // battleManager.queueForMatchmaking calls async getMatchmakingKey
  // getMatchmakingKey uses eloService to determine tier (including 'protected' for <10 battles)
  // from Plan 10-01
...
The key wiring chain is:
1. User selects stake (entryFee) in UI
2. socket.emit('queue_matchmaking', { config: { entryFee, ... } })
3. Backend socket handler calls battleManager.queueForMatchmaking(config, wallet)
4. queueForMatchmaking calls async getMatchmakingKey(config, wallet)
5. getMatchmakingKey uses eloService.shouldProtectPlayer() and eloService.getEloTier() from Plan 10-01
6. Returns tier-aware key like `0.1-180-real-protected` or `0.1-180-real-gold`
...
Verify wiring chain: `grep -n "queue_matchmaking" backend/src/index.ts` shows handler calls battleManager.queueForMatchmaking
Verify ELO integration: `grep -n "getMatchmakingKey" backend/src/services/battleManager.ts` shows async function using eloService
  </verify>
  <done>
BattleLobby UI passes user-selected stake amount to battleManager.queueForMatchmaking via socket.emit('queue_matchmaking', { config: { entryFee } }), which uses async getMatchmakingKey with ELO tier logic from Plan 10-01
  </done>
</task>

<task type="auto">
  <name>Task 5: Implement backend accept_challenge socket handler with battle creation</name>
  <files>
    backend/src/index.ts
    backend/src/services/battleManager.ts
  </files>
  <action>
Add the backend WebSocket handler for 'accept_challenge' event AND implement the createBattleFromChallenge method in battleManager.

**Part A: Add createBattleFromChallenge to battleManager.ts**

Add this method to the BattleManager class:

```typescript
/**
 * Create a battle from an accepted challenge
 * Composes existing createBattle and addPlayerToBattle methods
 */
async createBattleFromChallenge(
  ...
  const battle: Battle = {
    ...
    players: [],
    createdAt: Date.now(),
    prizePool: config.entryFee * 2,
  };

  // Store the battle
  this.battles.set(battleId, battle);

  // Add both players using existing addPlayerToBattle logic
  await this.addPlayerToBattle(battleId, challengerWallet);
  await this.addPlayerToBattle(battleId, accepterWallet);

  // Since both players are present, transition to active (or ready check)
  // For challenge-based battles, skip matchmaking queue and go straight to active
  battle.status = 'active';
  battle.startedAt = Date.now();

  logger.info('Battle created from challenge', {
    battleId,
...
**Part B: Add the socket handler in backend/src/index.ts**

Find the socket event handlers section (around line 2395 where challenge notification handlers are) and add:

```typescript
// Accept challenge via WebSocket
socket.on('accept_challenge', async (data: { code: string; walletAddress: string }) => {
  const { code, walletAddress } = data;

  challengeLogger.info('Challenge acceptance requested via WebSocket', { code, wallet: walletAddress });

  try {
    // Validate authentication
    const authenticatedWallet = getAuthenticatedWallet(socket, walletAddress);

    // Rate limit check
    const rateCheck = checkSocketRateLimit(socket.id, authenticatedWallet, 'accept_challenge', GAME_JOIN_LIMIT);
    if (!rateCheck.allowed) {
      socket.emit('challenge_error', { error: rateCheck.error || 'Rate limited' });
      return;
    }
    ...
    if (!challenge) {
      socket.emit('challenge_error', { error: 'Challenge not found' });
      return;
    }

    if (challenge.status !== 'pending') {
      socket.emit('challenge_error', { error: 'Challenge is no longer available' });
      return;
    }

    if (Date.now() > challenge.expiresAt) {
      socket.emit('challenge_error', { error: 'Challenge has expired' });
      return;
    }

    if (authenticatedWallet === challenge.challengerWallet) {
      socket.emit('challenge_error', { error: 'Cannot accept your own challenge' });
      return;
    }
    ...
    const updatedChallenge = challengesDb.acceptChallenge(code, authenticatedWallet, battleId);
    ...
    // Create the battle via battleManager using the new method
    ...
    // Use the new createBattleFromChallenge method
    ...
    if (notificationTarget?.socketId) {
      io.to(notificationTarget.socketId).emit('challenge_accepted', {
        battleId: battle.id,
        challengeCode: code,
    ...
    // Notify the accepter
    socket.emit('challenge_accepted', { battleId: battle.id });

    // Join both players to the battle room
    socket.join(`battle:${battle.id}`);

    // Emit battle_update to the room so both players get the battle state
    io.to(`battle:${battle.id}`).emit('battle_update', battle);

    challengeLogger.info('Challenge accepted successfully', {
      challengeId: challenge.id,
      battleId: battle.id,
      challenger: challenge.challengerWallet,
      accepter: authenticatedWallet,
    });

  } catch (error) {
    challengeLogger.error('Error accepting challenge via WebSocket', { error: String(error), code });
    socket.emit('challenge_error', { error: error instanceof Error ? error.message : 'Failed to accept challenge' });
  }
});
```
  </action>
...
Socket handler exists: `grep -n "socket.on('accept_challenge'" backend/src/index.ts` returns line number
createBattleFromChallenge exists: `grep -n "createBattleFromChallenge" backend/src/services/battleManager.ts` returns line number
  </verify>
  <done>
Backend handles 'accept_challenge' socket event, validates challenge, creates battle using new createBattleFromChallenge method in battleManager, notifies both players via WebSocket
  </done>
</task>

<task type="checkpoint:human-verify" gate="blocking">
  <what-built>Challenge board page with listing, filtering, creation, direct challenges, full matchmaking/challenge acceptance wiring, and backend socket handler with createBattleFromChallenge implementation</what-built>
  <how-to-verify>
...
9. Check backend logs for 'accept_challenge' handler being called
10. Verify createBattleFromChallenge method exists in battleManager.ts
  </how-to-verify>
...
- [ ] queue_matchmaking handler calls battleManager.queueForMatchmaking
- [ ] queueForMatchmaking uses async getMatchmakingKey with ELO tier logic
- [ ] Backend accept_challenge socket handler exists and works
- [ ] createBattleFromChallenge method exists in battleManager.ts
</verification>
...
6. /battle page reads challenge query param and calls backend acceptChallenge via WebSocket
...
10. queue_matchmaking wires through to getMatchmakingKey which uses ELO tier logic from Plan 10-01
11. Backend socket.on('accept_challenge') handler creates battle using createBattleFromChallenge method
</success_criteria>
