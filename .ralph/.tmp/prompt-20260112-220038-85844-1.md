...
Story details:
### [ ] US-001: Backend Persistence
Add database persistence for prediction bets and history API endpoints.

**Tasks:**
- T200: Save bets to database when placed, call userStatsDb.recordWager()
- T201: Add GET /api/predictions/history/:wallet and /api/predictions/round/:roundId

**Files:** backend/src/services/predictionService.ts, backend/src/database/userStats.ts, backend/src/routes/predictions.ts

**Verify:** Bets persist across server restart, frontend can fetch historical bets
...
4. Read /Users/taylermelvin/Desktop/sol-battles/.ralph/IMPLEMENTATION_PLAN.md and locate the section for US-001.
   - If no section exists, create `### US-001: Backend Persistence` and add the tasks needed.
...
9. Update /Users/taylermelvin/Desktop/sol-battles/.ralph/IMPLEMENTATION_PLAN.md:
   ...
   - Add notes or new tasks only within the US-001 section.
10. Update the PRD:
   - Check off **all acceptance criteria** for US-001 (`- [x]`) once verified.
   - Only after all acceptance criteria are checked, mark the story heading as complete
     (change `### [ ] US-001:` to `### [x] US-001:`).
...
```
## [Date/Time] - US-001: Backend Persistence
